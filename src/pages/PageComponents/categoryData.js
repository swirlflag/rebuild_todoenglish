...
const data = [
...

    {
        name : '버튼/Download',
        title : '스토어 다운로드 버튼',
        text : '앱스토어와 플레이 스토어 다운로드 버튼으로 [버튼/Default] 에서 파생되었습니다. 타입에 맞는 아이콘과 컨텐츠에 맞는 폰트 svg를 기본 내장합니다. 모바일 타입의 경우 IOS,Android 판별에 따라 이동합니다.',
        info : {
            '파생'                              : '버튼/Default',
            '링크: 앱스토어 '                   : 'TBD',
            '링크: 플레이 스토어 '              : 'TBD',
    ...
    {
        name : '입력/Radio Selector Collection',
...
// InputTextPassword
...

    {
        unique : true,
        name : '모달/Roof Alert',
        title : 'Roof Alert title',
        text : 'Roof Alert text',
        ...
        control : {
            ...
            call(payload) { {payload}
                if(!VM.$store.state.$modal.is_openRoofAlert){
                    VM.$store.dispatch('openRoofAlert' , payload);
                }else {
                    VM.$store.dispatch('closeRoofAlert');
                }
            },
        },
